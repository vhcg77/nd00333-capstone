...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="l2",
        help="Used to specify the norm used in the penalization. The ‘newton-cg’, ‘sag’ and ‘lbfgs’ solvers support only l2 penalties. ‘elasticnet’ is only supported by the ‘saga’ solver. If ‘none’ (not supported by the liblinear solver), no regularization is applied.",
    )
    parser.add_argument(
        "--C",
        type=float,
    ...
    model = LogisticRegression(
        penalty=args.penalty, C=args.C, max_iter=args.max_iter, solver=args.solver
    )
